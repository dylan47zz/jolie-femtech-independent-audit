...
out=Path('三个文件独立终审核查报告_Jolie_FemTech_2026-09-07.md')
models=json.loads(Path('independent_models.json').read_text())

def f(x,d=3):
    if abs(x)<0.00005 and x!=0:return '<0.0001'
    return f'{x:.{d}f}'
def pval(x):
    return '<0.0001' if x<0.0001 else f'{x:.4f}'
term_cn={'Intercept':'截距','政治体制代码_n':'政治体制代码（每升1类）','logGDP（公式）_n':'logGDP','cyearc':'CPR年份−2018','d1':'体制1：选举型威权 vs 0类','d2':'体制2：选举民主 vs 0类','d3':'体制3：自由民主 vs 0类'}
name_cn={'M1':'M1 log(MMR)~有序体制+logGDP','M2':'M2 MMR原始尺度~有序体制+logGDP','M3':'M3 CPR~有序体制+logGDP','M4':'M4 CPR~有序体制+logGDP+年份中心化','M5':'M5 CPR（2015–2023）~有序体制+logGDP','M6':'M6 log(MMR)~体制虚拟变量+logGDP','M7':'M7 CPR~体制虚拟变量+logGDP'}
model_rows=[]
for z in models:
    code=z['label']
    for r in z['rows']:
        model_rows.append('| {model} | {term} | {n} | {k} | {df} | {beta} | {ose} | {op} | [{ol},{ou}] | {hse} | {hp} | [{hl},{hu}] | {r2} | {ar2} |'.format(
            model=name_cn[code],term=term_cn[r['term']],n=z['n'],k=z['k'],df=z['dof'],beta=f(r['beta']),ose=f(r['ols_se']),op=pval(r['ols_p']),ol=f(r['ols_ci_l']),ou=f(r['ols_ci_u']),hse=f(r['hc3_se']),hp=pval(r['hc3_p']),hl=f(r['hc3_ci_l']),hu=f(r['hc3_ci_u']),r2=f(z['r2']),ar2=f(z['adjr2'])))

report=f'''# 三个文件独立终审核查报告
